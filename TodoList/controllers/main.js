...
const postUserData = (req, res, db) => {
  const { username, email, password, date_created, last_login } = req.body
  const date_created = new Date()
  db('users').insert({ username, email, password, date_created, last_login })
    .returning('*')
    .then(item => {
      res.json(item)
    })
...
const putUserData = (req, res, db) => {
  ...
  db('users').where({ uid }).update({ username, email, password })
    .returning('*')
    .then(item => {
      res.json(item)
    })
